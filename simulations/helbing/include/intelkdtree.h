#ifndef HELBING_INTELKDTREE_H_
#define HELBING_INTELKDTREE_H_
...
struct KDTreeSlidingMidpointNode{
  ...
  float bbminX, bbminY, bbmaxX, bbmaxY; // bounding box
  bool leaf_node;
...
struct KDTreeSlidingMidpoint{
  ...
  KDTreeSlidingMidpoint(unsigned maxnodes_, unsigned numAgents){
    ...
    for(unsigned i = 0; i < maxnodes; i++) {
      ...
      tree[i].numagents = 0;
      tree[i].bbminX = FLT_MAX; tree[i].bbminY = FLT_MAX; tree[i].bbmaxX = -FLT_MAX; tree[i].bbmaxY = -FLT_MAX;
...
/* 				     unsigned numAgents); */
  void generateKDTreeSlidingMidpointRecursive(int node, int readID, float * agentxcoord, float * agentycoord,
  ...
  void setNeighborsRecursive(float * agentxcoord, float * agentycoord, unsigned int agentindex, float radius, int k, Agent * outputagents[], float * outputagentsdist, unsigned & numoutputs, int node, float & mindist);
...
//printf("!RealID=%d (%d)\n",tp->realID,threadid); fflush(stdout);
    kdtree->generateKDTreeSlidingMidpointRecursive(tp->node,tp->realID,tp->agentxcoord,tp->agentycoord,tp->numAgents,tp->depth,threadid);
}
#endif
