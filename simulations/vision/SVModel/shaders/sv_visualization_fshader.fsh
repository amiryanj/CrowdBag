...
vec4 alphadColor(float bearingAngle)
{
    float v = (bearingAngle == 0.0) ? 0.0 : abs(bearingAngle)/PI;
    
    return green*v + (1.0-v)*red;
}

vec4 ttiColor(float tti)
{
    float v = (tti == 0.0) ? 0.0 : tti/10.0;
...
void main()
{
    ...
    gl_FragColor = ttiColor(ttc);
    //gl_FragColor = vec4(theta_Dot_1, theta_Dot_2, ttc, first);
}
